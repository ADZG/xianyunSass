...
export default {
  ...
  methods: {
    ...
    setDataList(arr) {
      ...
      const start = (this.pageIndex - 1) * this.pageSize; //分页是从1开始，而数组分隔是从索引0开始, 得到分割开始的索引
      const end = start + this.pageSize; //得到分割结束的索引
      this.dataList = this.flightsData.flights.splice(start, end); //找到当前机票数据列表,切割后返回一个新的数组
    },
    //    切换条数触发
    handleSizeChange(v) {
      this.pageSize = v;
